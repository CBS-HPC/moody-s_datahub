...
- run helper workflows such as indexed RapidFuzz company-name matching, BvD
  change tracking, and Orbis-to-DataHub column mapping
...
- session setup: `Sftp(...)`, `download_root`, `output_root`, `interactive`,
  `offline`, `tables_available()`, `set_data_product`, `set_table`,
  `select_data()`
...
`company_suffix()`, and `offline_capabilities()`.

```python
from moodys_datahub import Sftp

SFTP = Sftp(offline=True, interactive=False)
SFTP.search_dictionary(search_word="revenue")
SFTP.offline_capabilities()
```

By default, `bvd_list` validates values against known country prefixes and a
BvD-like ID pattern. In non-interactive scripts, invalid-looking values raise a
`ValueError`. Set `allow_invalid_bvd_ids=True` to keep them and treat them as
exact IDs:

```python
SFTP = Sftp(interactive=False, allow_invalid_bvd_ids=True)
